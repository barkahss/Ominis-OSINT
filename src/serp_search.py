...
def search_with_serpapi(query, language=None, country=None, date_range=None, num_results=100):
    ...
    # Validate country code - SerpAPI only accepts 2-letter country codes
    valid_country_codes = ['us', 'uk', 'ca', 'au', 'in', 'de', 'fr', 'es', 'it', 'jp', 'br', 'mx', 'ru', 'za', 'ar', 'co', 'id', 'my', 'ph', 'sg', 'th', 'tr', 'vn']
    if country:
        # Convert to lowercase and remove any 'lang_' prefix if accidentally used
        country_code = country.lower().replace('lang_', '')
        # Check if it's a valid 2-letter country code
        if len(country_code) == 2 and country_code in valid_country_codes:
            params["gl"] = country_code
        else:
            logger.warning(f"Unsupported country code: {country}. Using default.")
            print(f"{Fore.YELLOW}Warning: Unsupported country code '{country}'. Using default.{Style.RESET_ALL}")
